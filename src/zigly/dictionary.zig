...
pub const Dictionary = struct {
    ...
    pub fn open(name: []const u8) !Dictionary {
        ...
        try fastly(wasm.mod_fastly_dictionary.open(@ptrCast([*]const u8, name.ptr), name.len, &handle));
    ...
    pub fn get(self: Dictionary, allocator: *Allocator, name: []const u8) ![]const u8 {
        ...
        while (true) {
            const ret = wasm.mod_fastly_dictionary.get(self.handle, @ptrCast([*]const u8, name.ptr), name.len, @ptrCast([*]u8, value_buf.ptr), value_len_max, &value_len);
