...
pub const Logger = struct {
    ...
    pub fn open(name: []const u8) !Logger {
        ...
        try fastly(wasm.mod_fastly_log.endpoint_get(@ptrCast([*]const u8, name.ptr), name.len, &handle));
    ...
    pub fn write(self: *Logger, msg: []const u8) !void {
        ...
        try fastly(wasm.mod_fastly_log.write(self.handle, @ptrCast([*]const u8, msg.ptr), msg.len, &written));
    }
};
